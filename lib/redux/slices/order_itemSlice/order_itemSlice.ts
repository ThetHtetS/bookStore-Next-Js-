    ...
    reducers:
     {
      ...
      deleteOrder_item:(state,action:PayloadAction<Order_item>)=> {
        state.order_items = state.order_items.filter(item=> item._id != action.payload._id);
     },
     updateOrder_item:(state,action:PayloadAction<Order_item>)=> {
      state.order_items = state.order_items.map(item=> item._id == action.payload._id?action.payload: item);
   },
    },
